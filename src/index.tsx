...
function App() {
  ...
  const [selectedPhase, setSelectedPhase] = React.useState<number | undefined>();

  const [typeOfWork, setTypeOfWork] = React.useState<TypeOfWork>(TypeOfWork.Create);
    ...
    <>
      {
        user ? (
                  ...
                  <>
                    <Form.Item label="Date range">
                      <RangePicker
                        ...
                        disabledTime={(_) => {
                          return {
                            disabledHours: () => Array.from({ length: 23 }, (_, i) => i + 1),
                        ...
                        onChange={(dates) => setDates([dates[0].startOf('day'), dates[1].startOf('day')])}
                      />
                    </Form.Item>
                        ...
                        <>
                          <List
                            ...
                            renderItem={(item: string) => (
                              <List.Item>{dayjs(item).format("ddd, DD/MM/YYYY")}</List.Item>
                            )}
                          />
                          <Button danger onClick={(_e: any) => {
                    ...
                    try {
                      ...
                      if (isAutoFill) {
                        ...
                        while (currDate.isSameOrBefore(endDate)) {
                          if (currDate.day() > 0 && currDate.day() < 6 && !ignoreDates.includes(currDate.format('YYYY-MM-DD')))
                            tasks.push({
          ...
          <p>Please login to <span><a href="https://jira.fis.com.vn" target="_blank">JIRA</a></span> first then re-open this extension</p>
        )
      }
    </>
  );
